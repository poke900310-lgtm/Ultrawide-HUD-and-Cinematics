...
**PROVEN — the runtime plumbing works.** `WBP_GameHUD_C` can be acquired with
`FindAllOf` and re-acquired after a HUD rebuild when a cached instance stops being
`IsValid`; a widget's transform reaches Slate by writing `RenderTransform` then
calling `SetRenderTransformAngle` once (the only setter that flushes the whole
transform). Reading the live viewport and aspect ratio to recentre a box-aligned
HUD is the part a Lua mod must add on top.
